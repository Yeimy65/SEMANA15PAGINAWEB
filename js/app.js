...
(function menuHamburguesa() {
  ...
  nav.querySelectorAll("a").forEach(function(e) { e.addEventListener("click", function() { nav.classList.remove("abierto"); }); });
})();
...
(function paginaProductos() {
  ...
  fetch("data/productos.json")
    ...
    .then(function(datos) { todosLosProductos = datos; mostrarProductos(datos); })
    .catch(function() { contenedor.innerHTML = '<p class="sin-resultados" style="color:#dc2626;">⚠ Error al cargar los productos.</p>'; });
  ...
  for (var i = 0; i < botones.length; i++) {
    botones[i].addEventListener("click", function() {
      ...
      if (categoriaActual === "Todos") { mostrarProductos(todosLosProductos); }
      else { mostrarProductos(todosLosProductos.filter(function(p) { return p.categoria === categoriaActual; })); }
    });
  }
})();
...
function mostrarProductos(lista) {
  ...
  if (conteoEl) conteoEl.textContent = lista.length === 1 ? "1 producto encontrado" : lista.length + " productos encontrados";
  var html = "";
  for (var i = 0; i < lista.length; i++) {
    ...
    if (p.imagen && p.imagen !== "") {
      imgHtml = '<img src="' + p.imagen + '" alt="' + p.nombre + '" class="producto-img" onerror="this.style.display=\'none\';document.getElementById(\'fb-' + p.id + '\').style.display=\'flex\'">' +
...
(function paginaCompra() {
  ...
  fetch("data/productos.json")
    ...
    .catch(function(err) { console.error("Error JSON compra:", err); });
...
function actualizarResumen() {
  ...
  var cd = document.getElementById("campo-detalle");
  var cs = document.getElementById("campo-subtotal");
  var ct = document.getElementById("campo-total");
  var cf = document.getElementById("campo-fecha");
  if (cd) cd.value = prod.nombre + " x" + cantidad + " = $" + subtotal.toLocaleString("es-CL");
  if (cs) cs.value = "$" + subtotal.toLocaleString("es-CL");
  if (ct) ct.value = "$" + subtotal.toLocaleString("es-CL");
  if (cf) cf.value = fecha;
}
...
function validarFormCompra() {
  ...
  if (telefono !== "" && !esTelefonoValido(telefono)) { ponerError("err-telefono","Solo números (7-15 dígitos)."); marcarError("telefonoCliente"); hayError = true; }
  ...
  if (isNaN(cantidad) || cantidad <= 0) { ponerError("err-cantidad","La cantidad debe ser mayor a 0."); marcarError("cantidadInput"); hayError = true; }
  else if (idProd !== "") {
    var prodElegido = null;
    for (var i = 0; i < todosLosProductos.length; i++) {
      if (todosLosProductos[i].nombre === idProd) { prodElegido = todosLosProductos[i]; break; }
    }
    if (prodElegido && cantidad > prodElegido.stock) { ponerError("err-cantidad","Solo hay " + prodElegido.stock + " unidades."); marcarError("cantidadInput"); hayError = true; }
    else { marcarOk("cantidadInput"); }
  }
  if (hayError) ponerError("err-general","Corrige los campos marcados en rojo.");
  return !hayError;
}
...
(function paginaContacto() {
  ...
  if (txtMensaje && spanContador) {
    txtMensaje.addEventListener("input", function() {
      ...
      if (largo >= 20) spanContador.classList.add("ok"); else spanContador.classList.remove("ok");
      limpiarErrores(["cerr-mensaje"]); quitarMarcas(["cMensaje"]);
    });
  }
  formContacto.addEventListener("submit", function(ev) { ev.preventDefault(); if (validarFormContacto()) this.submit(); });
})();
...
function validarFormContacto() {
  ...
  if (correo === "")       { ponerError("cerr-correo","El correo es obligatorio.");                                       marcarError("cCorreo");  hayError = true; }
  ...
  if (mensaje.length < 20) { ponerError("cerr-mensaje","Mínimo 20 caracteres. Tiene " + mensaje.length + " actualmente."); marcarError("cMensaje"); hayError = true; } else { marcarOk("cMensaje"); }
  return !hayError;
}

function esCorreoValido(c)  { return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(c); }
function esTelefonoValido(t){ return /^[0-9+\s\-]{7,15}$/.test(t); }
function ponerError(id, msg){ var el=document.getElementById(id); if(el) el.textContent=msg; }
function limpiarErrores(l)  { for(var i=0;i<l.length;i++){ var el=document.getElementById(l[i]); if(el) el.textContent=""; } }
...
function quitarMarcas(l)    { for(var i=0;i<l.length;i++){ var el=document.getElementById(l[i]); if(el){ el.classList.remove("campo-error"); el.classList.remove("campo-ok"); } } }